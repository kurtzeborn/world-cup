import { PicksEntity, Results, ScoreEntity } from './types.js';
  ...
  try {
    ...
    for (const [groupId, predictedOrder] of Object.entries(groupPicks)) {
      ...
      for (let pos = 0; pos < 4; pos++) {
        ...
        if (actualPos === pos) {
          // Exact position match (positions 1st, 2nd, or 3rd) — always scores 3 pts
          // 4th place is excluded above (pos === 3 continue); 3rd-place exact scores
          // regardless of whether the team advanced to the knockout round.
...
export function buildRoundMap(matchResults: Record<string, { winner: string; loser: string }>): Record<string, Set<string>> {
...
export function getRoundFromMatchKey(matchKey: string): string | null {
...
export function getRoundForSlot(roundName: string): string {
  return roundName; // R32, R16, QF, SF, TPM, F
}

/** Get full and partial credit points for a round */
export function getRoundPoints(roundName: string): { full: number; partial: number } {
  switch (roundName) {
