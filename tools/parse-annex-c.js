// Parse the Wikipedia Annex C table rows and generate third-place-table.js
// Run: node tools/parse-annex-c.js > web/src/data/third-place-table.js

// All 495 rows from Wikipedia: https://en.wikipedia.org/wiki/2026_FIFA_World_Cup_knockout_stage
// Format: row# | A | B | C | D | E | F | G | H | I | J | K | L | [sep] | M74 | M77 | M79 | M80 | M82 | M81 | M85 | M87
// Letter in group column = that group's 3rd-place advances
// Slot columns: group letter of the 3rd-place team filling that slot (prefixed with "3" in source)

const RAW = `1 |   |   |   |   | E | F | G | H | I | J | K | L |   | 3E | 3J | 3I | 3F | 3H | 3G | 3L | 3K
...
for (const line of RAW.trim().split('\n')) {
  ...
  // parts[0] = row#, parts[1..12] = groups A-L, rest = slot values
  const advancing = [];
  for (let i = 0; i < 12; i++) {
    if (parts[i + 1] !== '') advancing.push(GROUPS[i]);
  }
  // Slot values are in the cells after index 12 (past all 12 group columns)
  // They start with '3' (e.g. "3E" = group E's 3rd-place team)
  const slots = parts.slice(13).filter(p => p.startsWith('3')).map(p => p.slice(1));
  if (slots.length !== 8 || advancing.length !== 8) {
    process.stderr.write(`Row ${parts[0]}: advancing=${advancing.length}, slots=${slots.length}\n`);
    continue;
  }
...
process.stderr.write(`Total entries: ${entries.length}\n`);

// Check for duplicates
const seen = new Set();
for (const [key] of entries) {
  if (seen.has(key)) process.stderr.write(`Duplicate key: ${key}\n`);
  seen.add(key);
}

// Output JS file
const lines = [
  '// Third-place team bracket placement table — FIFA World Cup 2026',
...
lines.push('');
lines.push('/**');
lines.push(' * Get 3rd-place team match slot assignments for a given set of 8 qualifying groups.');
lines.push(' * @param {string[]} advancingGroups - Array of 8 group letters that qualified');
lines.push(' * @returns {string[]|null} Array of 8 group letters [M74, M77, M79, M80, M82, M81, M85, M87]');
lines.push(' */');
lines.push('export function getThirdPlacePlacements(advancingGroups) {');
lines.push('  const key = [...advancingGroups].sort().join(\'\');');
lines.push('  return THIRD_PLACE_TABLE[key] ?? null;');
lines.push('}');
lines.push('');
